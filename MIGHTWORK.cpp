...
double inputValidate(double time) // 24 hr; 630am = 6.30; 930pm = 21.30;
{
    ...
    {
        cout << "Error: a valid number must be entered. Try again: ";
        cin.clear();
...
int inputValidate(int letter)
{
    cin >> letter;

    while (!(letter == 'P' || letter == 'p' || letter == 'C' || letter == 'c'))
    {
        cout << "Error: enter 'Y' for Yes or 'N' for No. Try again: ";
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
    ...
    double& total_amount_saved)
{
    ...
    for (int i = 0; i < (total_days_on_trip - 1); i++) // 5 = 4 nights
    {
        ...
        hotel_fee = inputValidate(hotel_fee);
...
int main()
{
    ...
    // Expenses of Airfare
    cout << "How did you travel? Plane or Car? (P or p for Plane, C or c for Car" << endl;
    cin >> x;
    if (x == "P", x == "p") {
    ...
    if (x == "C", x == "c") {
        ...
        cout << "But the total amount for driving that far " << miles * CENTS_PER_MILE << " dollars." << endl;
    }
    cout << "How much was spent on parking?" << endl;
    cin >> choice;
