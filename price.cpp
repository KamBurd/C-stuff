...
int main()
{
	...
	cout << endl << "the highest is " << highest << endl;
	lowest = findLowestPrice(priceTable, rowsUsed, colsUsed);
	cout << endl << "the lowest is " << lowest << endl;
	return 0;
}
...
void getPrices(PriceType table, int& numOfRows, int& numOfCols)
{
	...
	for (int row = 0; row < numOfRows; row++)
	{
		...
		{
			cout << "Type in a price with 2 decimal points" << endl;
			cin >> table[row][col];
...
void printPrices(PriceType table, int numOfRows, int numOfCols)
{
	...
	for (int row = 0; row < numOfRows; row++)
	{
		for (int col = 0; col < numOfCols; col++)
		{
			cout << setw(10) << table[row][col] << endl;
...
float findHighestPrice(PriceType table, int numOfRows, int numOfCols)
{
	...
	//make first element the highest price
	for (int row = 0; row < numOfRows; row++)
	{
		for (int col = 0; col < numOfCols; col++)
		{
...
//Exercise 2: Because gePrices is made to read in the input and printPrices is made to output
//Exercise 5: The highest is 102.34
//            The lowest is 1.45
